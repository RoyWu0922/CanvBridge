...
- `due_at` 用空串表示无截止（与 `get_todo` 同惯例）。
- 排序：`due_at` 升序，无截止排最后。
- 只保留 `published` 为真的项。
...
- `author` 取 `topic.author.display_name`，缺失给空串。
- `read_state` 取 `"read" | "unread"`，缺失按 `"read"` 处理（宁可不报未读，也不误报）。
- 排序：`pinned` 优先，其次 `last_reply_at` 降序（无则用 `posted_at`）。
...
**归一化必须按 `plannable_type` 分别取字段**（这是本稿最大的实现陷阱）：

| `plannable_type` | 标题来源 | 日期来源 | 是否有 `submissions` |
|---|---|---|---|
| `assignment` | `plannable.title` | `plannable_date` | 有 |
| `quiz` | `plannable.title` | `plannable_date` | 有 |
| `discussion_topic` | `plannable.title` | `plannable_date` | 无 |
| `wiki_page` | `plannable.title` | `plannable_date` | 无 |
| `calendar_event` | `plannable.title` | `plannable_date` | 无 |
| `planner_note` | `plannable.title` | `plannable.todo_date` | 无 |
| 其它（未知类型） | `plannable.title` | `plannable_date` | 无 |

- `date` 优先取 `plannable_date`，缺失再依次退到 `plannable.due_at` / `plannable.todo_date`，
  全缺则**丢弃该项**（无日期的聚合项对「本周 DDL」无意义）。
- `submitted` 只在有 `submissions` 对象时为布尔，否则 `None`（区别于 `False` —— 「未交」与
  「不适用」必须可区分）。
- `type` 直接透传 `plannable_type`，前端按它决定措辞。
- `course_name` 取 `context_name`（缺失给空串）。
...
- 未读徽章：`refreshBadges()` 增加第三项 `setNavBadge("discuss", countUnreadDiscussions())`。
  `countUnreadDiscussions()` 统计已加载结果里 `read_state === "unread"` 的条数；**首次加载
  讨论之前不显示徽章**（本稿不为讨论新增启动期后台预取 —— 公告与待办有 `bgFetch…`，
  讨论没有，也不加）。
...
- 正文以纯文本渲染（`esc()` 转义后插入），不注入 Canvas 返回的原始 HTML。
...
| 场景 | 行为 |
|---|---|
| 某门课的 quizzes / discussions 失败 | 进 `errors`，该门显示为空，其余课程不受影响 |
| Pages 拉取失败 | 详情弹层里该板块显示错误行，Modules / 文件照常 |
...
`tests/test_canvas_client.py` 加：5 个新函数的字段映射与排序测试，**重点覆盖
`get_planner_items` 的 `plannable_type` 分支表**（每种类型各一例 + 未知类型 + 无日期丢弃
+ `submitted` 三态）。

`tests/test_main.py` 加：5 个新端点的成功路径 + 失败路径（`ok: False`）+ 批量端点的
逐课程错误隔离。沿用现有 `monkeypatch` 风格。
...
4. 课表页：测验分组出现在考试区，AIMS 未登录也能看到
5. 首页仪表盘：统计卡 ×4 + 今日课程卡组 + 速览卡 ×3
6. 首页「本周 DDL」来自 planner，已交项不计入
...
| 风险 | 缓解 |
|---|---|
| **`planner/items` 字段随类型而异**，归一化漏一个类型就静默丢数据 | §5.1 的分支表写死 + 每种类型一条测试；未知类型走兜底分支而非丢弃 |
...
| token 权限不足（quizzes / pages / discussions 读不到） | 失败走 `errors`，单门失败不影响其余；不臆造数据 |
